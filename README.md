...
Tensora has a central class `Tensor` that wraps a C tensor structure managed by `cffi`. Tensora exposes functions that take a string of an algebraic expression and return a Python function that performs that operation in fast machine code. In order to do that, the string is parsed and passed to the tensor algebra compiler. The code generated by the compiler is compiled "on the fly" and then wrapped by a Python function that provides good error handling. Tensora's tensor algebra compiler, the algorithm that converts algebraic expressions of tensors into code, takes inspiration from TACO, but is a new algorithm written in Python.
