...
from tensora.expression import InconsistentDimensionsError, MutatingAssignmentError, ast
...
def assignments(draw):
    ...
    except (InconsistentDimensionsError, MutatingAssignmentError):
        assume(False)
...
@st.composite
def formats(draw, orders=st.integers(min_value=0, max_value=16)) -> Format:
    order = draw(orders)
...
def tensors(draw, format: Format, dimensions: tuple[int, ...] | None = None) -> Tensor:
    ...
    dok = draw(
        st.dictionaries(
            st.tuples(*[st.integers(min_value=0, max_value=dim) for dim in dimensions]),
